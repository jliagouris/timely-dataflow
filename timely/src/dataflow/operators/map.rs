...
pub trait Map<'a, S: Scope<'a>, D: Data> {
    ...
    /// });
    /// ```
    fn map<D2: Data>(&self, logic: impl Fn(D)->D2+'static) -> Stream<'a, S, D2>;
    ...
    /// });
    /// ```
    fn map_in_place(&self, logic: impl Fn(&mut D)+'static) -> Stream<'a, S, D>;
    ...
    /// });
    /// ```
    fn flat_map<I: IntoIterator>(&self, logic: impl Fn(D)->I+'static) -> Stream<'a, S, I::Item> where I::Item: Data;
}

impl<'a, S: Scope<'a>, D: Data> Map<'a, S, D> for Stream<'a, S, D> {
    fn map<D2: Data>(&self, logic: impl Fn(D)->D2+'static) -> Stream<'a, S, D2> {
