...
pub trait Accumulate<'a, G: Scope<'a>, D: Data> {
    /// Accumulates records within a timestamp.
    ///
    /// # Examples
    ///
    ...
    /// ```
    fn accumulate<A: Data>(&self, default: A, logic: impl Fn(&mut A, RefOrMut<Vec<D>>)+'static) -> Stream<'a, G, A>;
    ...
    /// ```
    fn count(&self) -> Stream<'a, G, usize> {
        self.accumulate(0, |sum, data| *sum += data.len())
    }
}

impl<'a, G: Scope<'a>, D: Data> Accumulate<'a, G, D> for Stream<'a, G, D> {
    fn accumulate<A: Data>(&self, default: A, logic: impl Fn(&mut A, RefOrMut<Vec<D>>)+'static) -> Stream<'a, G, A> {
